...
#### Programming
Programmings were carried out in three programming languages MATLAB, Python, and C++. The first code that was created `Generator.m` uses MATLAB Symbolic Toolbox to find the symbolic discretization of the railway track PDE and return a symbolic set of ordinary differential equations (ODEs). The script later uses `MatlabFunction` to generate matlab function that are optimized for next computation.
...
The next piece of programming is the script `Plotter.m` is created to produce readable and informative plots.
...
Finally, all the previous components are put together in one single main file named `main.m`. In the following, I have included this last piece written in Python `main.py` to provide you with a comparison of Python with MATLAB.
