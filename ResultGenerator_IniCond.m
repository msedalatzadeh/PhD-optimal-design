...
mesh=3500
cu=10
z0=[2,3,2/2,3/2,2/4,3/4,2/8,3/8,2/16,3/16,2/32,3/32]'
T=10
p=parpool(15);
p.IdleTimeout=Inf;
%% Constants
...
options=optimoptions('fmincon','Display','off','SpecifyObjectiveGradient'...
   ,true,'HonorBounds',true,'Algorithm',...
...
[u_mid,J_mid]=fmincon(@(u)RTcostgrad_u(u,l/2,par),u_fmin,[],[],[],[]...
   ,lb(1:m),ub(1:m),[],options);
[z_mid,~,~,~,~]=RTsolver([u_mid,l/2],par);
w_mid=zeros(1,m);
parfor i=1:m
   for j=1:(n/2)
      w_mid(i)=w_mid(i)+(z_mid(2*j-1,i)-z_mid(2*j,i))*c(j,par{1})*1/(j^2*pi^2)*sin(j*pi/2);
   end
end
%% Initial Condition
...
% end
%%
save(sprintf('%gmode&Alpha%g&Cd%g&mu%giterInfcu10.mat',N,Alpha,Cd_exp,mu_exp))
